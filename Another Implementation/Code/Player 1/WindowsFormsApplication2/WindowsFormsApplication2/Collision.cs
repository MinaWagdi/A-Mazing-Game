...
namespace WindowsFormsApplication2
{
    class Collision
    {
        ...
        {
            Color check = GamePanel.maze.GetPixel((int)c.pos.X, (int)c.pos.Y + 19);
            bool check1 = isWhitePixel(check);
            check = GamePanel.maze.GetPixel((int)c.pos.X + 15, (int)c.pos.Y + 19);
            bool check2 = isWhitePixel(check);
            return check1 && check2; 
        }
        public static bool canMoveRight(DynamicImage c)
        {
            Color check = GamePanel.maze.GetPixel((int)c.pos.X + 19, (int)c.pos.Y);
            bool check1 = isWhitePixel(check);
            check = GamePanel.maze.GetPixel((int)c.pos.X + 19, (int)c.pos.Y +15);
            bool check2 = isWhitePixel(check);
            return check1 && check2; 
        }
        ...
        public static bool MonsterCollision(DynamicImage c)
        {
            ...
            if (GamePanel.monsters.Capacity != 0)
            {
                foreach (Monster m in GamePanel.p1.monsters)
                {
                    Rectangle rectC = new Rectangle((int)c.pos.X, (int)c.pos.Y, Constants.BULLET_RAD, Constants.BULLET_RAD);
                    Rectangle rectMonster = new Rectangle((int)m.pos.X, (int)m.pos.Y, Constants.Monster_Length, Constants.Monster_Length);


                    if (rectC.IntersectsWith(rectMonster))
                    {
                        GamePanel.p1.monsters.Remove(m);
